...
```php
if ( $product->is_synced() && $user->can_sync_products() ) {
    $product->sync_to_pos();
}
```
...
if ( $product->is_synced() ) {
    ...
    if ( $user->can_sync_products() ) {
        // Sync the product to POS
        $product->sync_to_pos();
    }
}
```
...
- PayPal POS API-specific requirements or limitations
...
if ( $product->needs_sync() && ! $product->has_valid_sku() ) {
    $this->logger->log( 'Invalid product SKU' );
    $this->notify_admin( $product );
    return false;
} else {
    return true;
}
